...
<QBXML>
    ...
    def receive_purchase_order(
        ...
        """
        # Use QBFC for item receipt creation to handle job assignments properly
        ...
        <PurchaseOrderQueryRq requestID="1">
            ...
            if line_items:
                ...
                for receive_item in line_items:
                    ...
                    # Add the receipt line with CustomerRef from PO if present
                    customer_job = po_line.get('customer_job', '')

                    # For now, just create simple receipt line without CustomerRef or LinkToTxn
                ...
                for line in po_details.get('line_items', []):
                    ...
                    if remaining > 0:
                        # Get CustomerRef from PO line if present
                        customer_job = line.get('customer_job', '')

                        # Build LinkToTxn XML to link to the PO line
                        link_to_txn_xml = ''
                        po_line_id = line.get('txn_line_id', '')
                        if po_line_id:
                            link_to_txn_xml = f"""
                    <LinkToTxn>
                        <TxnID>{po_txn_id_actual}</TxnID>
                        <TxnLineID>{po_line_id}</TxnLineID>
                    </LinkToTxn>"""
                ...
                <ItemReceiptLineAdd>
                    ...
                    <Quantity>{remaining}</Quantity>{link_to_txn_xml}
                </ItemReceiptLineAdd>"""
...
<QBXML>
    <QBXMLMsgsRq onError="stopOnError">
        <ItemReceiptAddRq requestID="1">
            ...
            logger.info(f"ItemReceipt XML: {request_xml}")
            # Write to file for debugging
            with open('receipt_debug.xml', 'w') as f:
                f.write(request_xml)
